...
Este programa muesta el movimiento de un auto en un recorrido de foma de triangular
*/
...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(movKitX, 4, movKitZ); //importante
	glRotatef(rotKit, 0, 1, 0); //rotacion en "y"
...
void animacion() //recorrido del automovil
{
	fig3.text_izq -= 0.01;
	fig3.text_der -= 0.01;
	if (fig3.text_izq < -1)
	...
	if (circuito1)//recorrido 1 hasta el 5 eje x esta en profundidad y z en vertical y horizontal
	{
		if (recorrido1)
		{
			movKitZ++;
			if (movKitZ > 155) //mov a coordenada z = 155
			{
				recorrido1 = false;
				recorrido2 = true; //activo bandera 2 recorrido
			}
		}
		if (recorrido2)
		{
			rotKit = 90; //rotar mi carroceria 90 grados para que se mueva para variacion en x
			movKitX++;
			if (movKitX > 125) //hasta coordenada 125
			{
				recorrido2 = false;
				recorrido3 = true;
		...
		if (recorrido3)
		{
			rotKit = 180; //rotar 180 para que rote y empiece a abanzar
			movKitZ--; // z negativa
			if (movKitZ < -155) //hasta que llegue a posicion -155
			{
				recorrido3 = false;
				recorrido4 = true;
			}
		...
		{
			rotKit = 270;
			movKitX--; //regrese a posciion de 0
			if (movKitX < 0)
			{
				recorrido4 = false;
				recorrido5 = true;
	...
	if (circuito2)//recorrido 1 hasta el 5 eje x esta en profundidad y z en vertical y horizontal
	{
		if (recorrido1)
		{
			rotKit = 180;
		...
		if (recorrido3)
		{
			rotKit = 0; //rotar 180 para que rote y empiece a abanzar
			movKitZ++; // z negativa
			if (movKitZ > 155) //hasta que llegue a posicion -155
			{
				recorrido3 = false;
	...
	if (circuito3)//recorrido 1 hasta el 5 eje x esta en profundidad y z en vertical y horizontal
	{
		if (recorrido1)
		{
			rotKit = 180;
		...
		if (recorrido3)
		{
			rotKit = -21.95; //rotar 180 para que rote y empiece a abanzar
			movKitX -= 0.403; // z negativa
			movKitZ++;
			if (movKitZ > 155) //hasta que llegue a posicion -155
			{
				recorrido3 = false;
		...
		if (recorrido4)
		{
			movKitZ++; //regrese a posciion de 0
			if (movKitZ > 155)
			{
				recorrido4 = false;
				recorrido5 = true;
...
void keyboard(unsigned char key, int x, int y)  // Create Keyboard Function
{
	...
	case 'i':	//  
	case 'I':
		circuito1 ^= true; //Activamos/desactivamos la animacíon
		g_fanimacion = false;
		break;
	case 't':	//  
	case 'T':
